...
Built in 4 days (February 10-13, 2026), originally estimated at 4-6 weeks. This document tells the engineering story of migrating a home SOC from a QNAP-based v2 architecture to a purpose-built bare metal v3 platform. Phase 12 covers the subsequent infrastructure redesign (March 14-16, 2026). Each phase covers what was built, why specific decisions were made, what went wrong, and the measurable outcome.
...
## Phase 12: Infrastructure Redesign

**Duration:** March 14-16, 2026 (rack migration weekend)

### What Was Built

A dedicated bare metal detection host (haccp) was deployed, the ELK Stack was migrated from a constrained LXC container to that new hardware, an OpenCTI v7 threat intelligence platform was stood up, and legacy v2 infrastructure was decommissioned. This was the first major architectural change since the v3 migration completed in February, driven by two things: ELK had outgrown its LXC resource allocation, and the lab lacked a structured threat intelligence pipeline.

**haccp** is a Lenovo ThinkStation P340 Tiny (i7-10700T 8C/16T, 32GB DDR4, 2TB + 1TB NVMe) deployed on VLAN 30 at 10.10.30.25. It runs the full ELK Stack (Elasticsearch 8.17 with a 12GB heap, Kibana, Fleet Server, Logstash) and Arkime v6.0.1 for full packet capture and session analysis. The name continues the BBQ theme -- HACCP (Hazard Analysis and Critical Control Points) is the food safety framework pitmasters follow, and a detection-focused host that analyzes everything crossing the network felt like a fitting parallel.
...
**Bare metal over another LXC for ELK:** ELK LXC 201 had been allocated 6 vCPU and 10GB RAM on pitcrew, which was already tight. Elasticsearch's JVM heap was capped at 4GB, and Logstash had to be treated as on-demand rather than always-on. On haccp bare metal, Elasticsearch gets a 12GB heap with 20GB of free RAM for filesystem cache, Logstash runs continuously, and Arkime can be added without resource contention. The ThinkStation P340 Tiny was already on hand from a previous project -- no new hardware cost.

**OpenCTI v7 over manual IOC management:** Before OpenCTI, IOC enrichment in Shuffle WF1 relied solely on AbuseIPDB lookups at alert time. There was no structured feed of known malicious indicators, no CVE correlation, and no MITRE ATT&CK context beyond what was manually referenced. OpenCTI automates the ingestion, correlation, and distribution of threat intelligence from five authoritative sources, turning what was a reactive lookup into a proactive detection capability.

**Decommission v2 metrics stack entirely:** The InfluxDB + Telegraf + Grafana stack on smokehouse had been kept alive during the Phase 3 cutover because Proxmox infrastructure dashboards depended on it. By March, Prometheus on brisket had fully replaced this capability with node-exporter scrapes of all hosts. Keeping InfluxDB running on a 16GB NAS that was already tight on memory was no longer justified.

**End the honeypot campaign:** The GCP WordPress honeypot was an INST 570 research project that ran from February through March 12, 2026. The research objectives were met, the data was collected and analyzed (via Shuffle WF7 weekly reports and the Kibana honeypot dashboard), and continuing to run the honeypot consumed GCP resources and operational attention with diminishing returns. Historical data is preserved in the ELK honeypot indices on haccp.

### Challenges

**Fleet agent re-enrollment:** Moving Fleet Server from LXC 201 to haccp required re-enrolling all four Elastic Agents. The agents were configured with the old Fleet Server URL (10.10.30.23:8220) baked into their enrollment tokens. New enrollment tokens were generated from the haccp Fleet Server, and each agent was re-enrolled in sequence. The smokehouse and sear agents were straightforward (SSH access), but DC01 and WS01 required WinRM sessions to update the agent configuration and restart the Elastic Agent service.

**Arkime capture dependency on SFP+ transceiver:** Arkime v6.0.1 was installed and configured on haccp, but actual packet capture cannot start until an SFP+ RJ45 transceiver arrives for the MokerLink TE11 slot. This slot is reserved as a SPAN mirror destination. Until then, Arkime serves the viewer UI for session search against any future captures. This is a known gap documented for follow-up.

**pitcrew RAM relief and reallocation:** Shutting down ELK LXC 201 freed 10GB of RAM on pitcrew. OpenCTI LXC 202 was allocated 10GB, consuming most of that freed capacity. The net effect on pitcrew's RAM budget is roughly neutral, but the workload shifted from a detection engine (ELK) that had outgrown its allocation to a threat intelligence platform (OpenCTI) that fits comfortably within it.
...
- Wazuh agents increased from 10 to 12 (agent 014 haccp, agent 011 ELK/LXC 201 removed, agent 013 PBS added)
- Prometheus targets increased from 6 to 7 (haccp node-exporter added)
- v2 metrics stack decommissioned (Telegraf, InfluxDB, Grafana on smokehouse; Telegraf on pitcrew and smoker)
- Honeypot campaign ended 2026-03-12 (GCP services stopped, historical data preserved)
- ELK LXC 201 shut down on pitcrew (not destroyed -- available for rollback)
...
| Metric | Value |
|--------|-------|
| Total hosts monitored | 13 (12 agents + 1 syslog) |
| Wazuh detection rules | Built-in engine + custom decoders |
| Elastic detection rules | 1345 (on haccp bare metal) |
| SOAR workflows | 8 (WF1-WF8) |
| Threat intel connectors | 6 (OpenCTI v7 -- MITRE ATT&CK, CVE, AbuseIPDB, CISA KEV, Malpedia, AlienVault OTX) |
...
| Initial migration | 4 days (Feb 10-13, estimated 4-6 weeks) |
| Infrastructure redesign | 3 days (Mar 14-16) |
