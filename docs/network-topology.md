...
| 30 | 10.10.30.0/24 | 10.10.30.1 | Lab / Proxmox / AD | Hypervisors, AD domain, TheHive, ELK (haccp), OpenCTI, backup |
...
|----------|----|----------------------|------|
| pitcrew | 10.10.30.20 | ThinkStation P340 Tiny (i7-10700T, 32GB) | Proxmox host -- AD lab VMs, TheHive LXC, OpenCTI LXC |
...
| ~~ELK LXC 201~~ | ~~10.10.30.23~~ | ~~LXC on pitcrew (6 vCPU, 10GB)~~ | ~~Decommissioned -- ELK migrated to haccp bare metal (Phase 12)~~ |
| PBS LXC 300 | 10.10.30.24 | LXC on smoker (2 vCPU, 2GB) | Proxmox Backup Server (NFS to smokehouse 17TB) |
| **haccp** | **10.10.30.25** | **ThinkStation P340 Tiny (i7-10700T, 32GB, 2TB+1TB NVMe)** | **Detection & PCAP -- ELK 8.17, Arkime v6.0.1** |
...
|----------|----|----------|------|
| GCP VM | External (Tailscale overlay) | Google Cloud (us-east4) | brianchaplow.com + bytesbourbonbbq.com hosting, Wazuh agent (honeypot decommissioned 2026-03-12) |
...
| ELK data shipping | brisket (10.10.20.30) | haccp (10.10.30.25) | TCP 9200 | Drift metrics, workflow data |
...
| Prometheus all 7 targets | brisket | all hosts | ALL UP | ALL UP |
...
graph TB
    ...
    subgraph VLAN30["VLAN 30 -- Lab / Proxmox / AD (10.10.30.0/24)"]
        ...
        HACCP["haccp<br/>10.10.30.25<br/>ELK + Arkime (TE8)"]
        OPENCTI["OpenCTI LXC 202<br/>10.10.30.26"]
    ...
    style HACCP fill:#1a3a1a,stroke:#44ff44,color:#fff
```
...
graph LR
    subgraph DataSources["Data Sources"]
        AGENTS["12 Wazuh Agents<br/>(all VLANs)"]
    ...
    subgraph Lab["Lab Services (VLAN 30)"]
        ...
        ELKSTACK["ELK Stack (haccp)<br/>ES 8.17 + Kibana<br/>+ Fleet (1345 rules)<br/>:9200, :5601"]
        OCTI["OpenCTI v7<br/>Threat Intel<br/>:8080"]
...
1. **Collection:** 12 Wazuh agents across all VLANs ship security events to the Wazuh Manager on brisket via TCP 1514. OPNsense forwards firewall syslog via UDP 514. smokehouse runs Suricata and Zeek against SPAN traffic, with Suricata alerts shipped through the Wazuh agent and Zeek metadata shipped directly to OpenSearch via Fluent Bit.

2. **Detection:** Wazuh correlates events against custom rules and generates alerts stored in OpenSearch (`wazuh-alerts-*`). ELK on haccp runs 1345 Elastic detection rules against Fleet agent data independently. OpenCTI v7 syncs threat intelligence IOCs to Wazuh CDB lists for indicator-based detection.
...
graph LR
    ...
    style Cloudflare fill:#1a1a3a,stroke:#8888ff,color:#fff
```

### GCP Pipeline Details

The GCP VM hosts the public portfolio and BBQ blog sites behind Cloudflare. A Wazuh agent (009) ships OS-level security events to brisket's Wazuh Manager.

> **Honeypot campaign (decommissioned 2026-03-12):** A WordPress login honeypot previously ran on the GCP VM, capturing credential stuffing attempts. Fluent Bit shipped honeypot logs to the ELK stack via Tailscale, and a cron job on brisket (`honeypot-wazuh-sync.py`) synced Wazuh alerts to ELK's `honeypot-wazuh` index. The campaign ended March 12, 2026. GCP honeypot services have been stopped. Historical data is preserved in ELK honeypot indices on haccp.

**Why Tailscale?** The GCP VM has no direct route to the home network's RFC 1918 address space (10.10.x.x). Tailscale provides a WireGuard-based mesh VPN, assigning each node a stable 100.x.x.x address and handling NAT traversal automatically. No port forwarding or public exposure of internal services is required.
...
3. **Full visibility.** SPAN port mirroring captures all switch traffic for Suricata/Zeek analysis. 12 Wazuh agents cover every VLAN. Prometheus scrapes 7 infrastructure targets. Every layer generates telemetry.
...
5. **External integration without exposure.** The GCP VM connects home via Tailscale mesh VPN, requiring zero inbound port exposure on the home network. OpenCTI aggregates external threat feeds and distributes IOCs to internal detection systems.
