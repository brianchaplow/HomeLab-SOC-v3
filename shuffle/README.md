...
Shuffle SOAR platform running on **brisket** (10.10.20.30) with 9 active automation workflows (plus 1 decommissioned). The platform integrates with Wazuh, TheHive, Cortex, OpenCTI, the ML Scorer, ELK, Caldera, and Ollama (qwen3:8b) to provide automated enrichment, triage, response, and intelligence reporting.
...
| WF7 | ~~Honeypot Intel Report~~ (decommissioned March 2026) | ~~System cron~~ | ~~12:00 Sun~~ |
| WF8 | LLM Log Anomaly Finder | System cron | 15:00 |
| WF9 | Stalker Visit Alert | System cron | Every 5 min |
| CS1 | Capitol Signals Collector | System cron | 21:00 |
...
WF1 also integrates with **OpenCTI** for IOC enrichment via workflow variables `$opencti_url` and `$opencti_api_key`.
...
Six workflows (WF1, WF2, WF5, WF6, WF7, WF8) use Ollama with the `qwen3:8b` model for natural language analysis. Key implementation details:
